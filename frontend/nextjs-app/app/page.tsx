...
export default function Home() {
  ...
  async function fetchStoryboard(project: Project) {
    try {
      const res = await fetch(`${API_BASE_URL}/projects/${project.project_id}/storyboard`);
      const data = await res.json();
  ...
  async function fetchExistingStoryboards(projectList: Project[]) {
    for (const project of projectList) {
      await fetchStoryboard(project);
    }
  }
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/projects`)
      ...
      .then(async (data) => {
        ...
        await fetchExistingStoryboards(projectList);
      })
      .catch(() => setProjects([]));
  }, []);
  ...
  async function generateStoryboard(project: Project) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.detail || "Storyboard generation failed.");
      }
      ...
      <section className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="rounded-2xl border border-purple-900/50 bg-gradient-to-r from-purple-950/40 to-cyan-950/20 p-6">
          ...
          <p className="text-gray-300">
            ...
            download original files, and generate persistent AI storyboards.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-950/70 p-6 space-y-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              onClick={async () => {
                await fetchProjects();
                await fetchExistingStoryboards(projects);
              }}
                  ...
                  {storyboards[project.project_id] && (
                    <div className="rounded-lg border border-purple-900/50 bg-purple-950/20 p-4 space-y-3">
                      <h4 className="text-lg font-bold text-purple-300">
                        Persistent AI Storyboard
                      </h4>

                      {storyboards[project.project_id].storyboard.map((scene) => (
                        <div
                          key={`${project.project_id}-${scene.scene}`}
